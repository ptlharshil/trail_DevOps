...
var getRealCellValue = function (column, value, isHeader) {
    ...
    switch (column.dataType) {
        ...
        case Models_1.ColumnDataType.DateTimeUtc:
            return (0, Models_1.parseDateColumnValue)(column, value);
        case Models_1.ColumnDataType.Boolean:
...
var processRow = function (row, columns, isHeader) {
    var finalVal = columns.reduce(function (prev, currentColumn) {
        var result = "\"".concat(getCellValue(currentColumn, row, isHeader).replace(/"/g, '""'), "\"");
        return "".concat(prev).concat(result, ",");
    }, '');
    return "".concat(finalVal.replace(/(^,)|(,$)/g, ''), "\n");
};
...
var getHtml = function (gridResult, columns) {
    var exportableColumns = columns.filter(function (c) { return (c.visible && c.exportable) || (c.isComputed && !c.getComputedStringValue); });
    return "<table class=\"table table-bordered table-striped\"><thead><tr>".concat(exportableColumns.reduce(function (prev, el) { return "".concat(prev, "<th>").concat(el.label || el.name, "</th>"); }, ''), "</tr></thead><tbody>").concat(gridResult.reduce(function (prevRow, row) {
        return "".concat(prevRow, "<tr>").concat(exportableColumns.reduce(function (prev, currentColumn) {
            return "".concat(prev, "<td>").concat(getCellValue(currentColumn, row, false), "</td>");
        }, ''), "</tr>");
    }, ''), "</tbody></table>");
};
